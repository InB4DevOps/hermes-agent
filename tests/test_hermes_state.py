...
class TestClearMessages:
    ...
    def test_clear_messages(self, db: SessionDB):
        ...
        # Tool responses don't increment tool_call_count, only actual tool calls do
...
class TestConcurrentAccess:
    ...
    def test_concurrent_reads_writes(self, db: SessionDB):
        ...
        def reader():
            try:
                # Wait a bit for writer to potentially start creating sessions
                time.sleep(0.05)
                for i in range(10):
                    time.sleep(0.01)
                    sessions = db.search_sessions()
                    messages = db.message_count()
                    # Reader may see sessions being created by writer
                    assert len(sessions) >= 0  # At least 0 (should be 0-10 depending on timing)
                    assert messages >= 0  # At least 0
            except Exception as e:
                with lock:
                    errors.append(e)
